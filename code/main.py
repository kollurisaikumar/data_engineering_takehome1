...
from pyspark.sql.functions import (
    col, count, avg, max as spark_max, min as spark_min, desc, asc, 
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(f"Loading data from: {input_dir}/nyc-jobs.csv")
        jobs_df = spark.read.option("header", "true").option("inferSchema","true").csv(f"{input_dir}/nyc-jobs.csv")
        ...
        functions.write_single_csv(skill_salary_distribution_df, "Skill_Salary_Distribution",output_dir)

        # FILE 2: Degree Salary Correlation (KPI 3)
        degree_salary_correlation_df = functions.degree_salary_correlation(jobs_df)
        functions.write_single_csv(degree_salary_correlation_df,"Degree_Salary_Correlation",output_dir)

        # FILE 3: Agency Insights (KPI 4+5)
        agency_insights_df = functions.agency_insights(jobs_df)
        functions.write_single_csv(agency_insights_df,"Agency_insights",output_dir)

        # FILE 4: Highest paid skills (KPI 6)
        functions.write_single_csv(highest_paid_skills_df,"Highest_Paid_Skills",output_dir)
