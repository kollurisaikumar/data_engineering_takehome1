...
from pyspark.sql.functions import (
    col, lower, when, expr,
    concat_ws, coalesce,
    countDistinct, count, avg,
    lit, round as spark_round, length
    )
...
def categorize_job_qualification(df, req_col="Minimum Qual Requirements"):
    ...
    try:
        logger.info("Categorizing qualifications from column: %s",req_col)
        ...
        logger.error("Qualification categorization failed: %s",str(e))
        raise
    ...
    try:
        ...
        df_result = (df
            ...
            .withColumn("Salary_Qual_Flag",
                when(col("Annual_Salary_Mid") <
                     col("qual_median_salary") * 0.85,
                      "underpaid")
                .when(col("Annual_Salary_Mid") <
                      col("qual_median_salary") * 0.95,
                      "below standard")
                .when(col("Annual_Salary_Mid") <=
                      col("qual_median_salary") * 1.105,
                        "industry standard")
                .when(col("Annual_Salary_Mid") <=
                      col("qual_median_salary") * 1.115,
                        "above standard")
        ...
        logger.error("Salary qualification flagging failed: %s",str(e), exc_info=True)
        raise
...
def highest_paid_skills(df, min_jobs=3):
    """
        Analyzed the available skills and finds the highgest paid skill
    """
    try:
        ...
        all_skills = []
        for _, subcats in skills_json.items():
            for _, keywords in subcats.items():
        ...
        logger.info("  Generated %s top skills",final_stats.count())
        return final_stats
    except Exception as e:
        logger.error("Highest paid skills failed: %s",str(e))
        raise
