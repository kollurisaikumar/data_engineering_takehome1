...
from .logging_config import get_logger, setup_pipeline_logging
from pyspark.sql.types import (
    StructType, StructField, StringType, IntegerType, DoubleType, 
    TimestampType, DateType, LongType
)
from pyspark.sql.functions import (
    col, count, avg, max as spark_max, min as spark_min, desc, asc, lit,
    lower, regexp_extract, explode, split, size, struct, when, expr,
    round as spark_round, avg as spark_avg, countDistinct,to_date
)
...
def write_single_csv(spark_df, keyword, output_dir):
    ...
    try:
        ...
        """Write Spark DataFrame as single CSV file using PySpark"""
        ...
        if not part_files:
            raise Exception("No part file generated")
        ...
        logger.info(f"Saved single CSV: {final_path}")
    except Exception as e:
        logger.info(f"Unable to save single csv file : {final_path} : {str(e)}")
        raise
...
def skill_salary_distribution(df):
    try:
        ...
        logger.info(f"Unable to calculate skill salary distribution :{str(e)} ")
        raise
...
def degree_salary_correlation(df):  
    ...
    try:
        ...
        logger.info(f"unable to create salary degree correlation table : {str(e)}")
        raise
...
def agency_insights(df, min_jobs=3):
    ...
    try:
        ...
        if max_date is None:
            ...
            logger.info(f"Filtering: {cutoff_date} to {max_date}")
        ...
        logger.info(f"Global avg salary (2yr): {global_avg:.0f}")
        ...
        logger.info(f"Generated insights for {final_stats.count()} agencies")
        return final_stats
        
    except Exception as e:
        logger.error(f"Error in agency_insights_2yr: {str(e)}")
        raise
